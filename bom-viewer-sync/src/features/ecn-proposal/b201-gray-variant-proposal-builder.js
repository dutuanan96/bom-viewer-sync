const TARGET_COLOR = Object.freeze({ zh: '灰色', vi: 'màu xám' });

export const CHANGE_REASON = 'ECN-2026-0824-B201: 新增灰色 B201 SKU；沿用黑色 BOM，仅替换布抽及印有 SKU 的纸箱。';
...
function grayCode(sourceCode) {
  if (!sourceCode.endsWith('BH')) throw new Error(`Expected BH material code: ${sourceCode}`);
  return `${sourceCode.slice(0, -2)}GY`;
}

function grayNameVi(sourceName) {
  const text = String(sourceName || '');
  const replaced = text
    .replace(/đen khói/giu, 'xám')
    .replace(/màu đen/giu, 'màu xám');
  return replaced === text ? `${text} - màu xám` : replaced;
}
...
function buildMasterDataOperations(payload) {
  ...
  for (const sourceCode of fabricCodes) {
    ...
    const code = grayCode(sourceCode);
  ...
  for (const config of VARIANT_CONFIGS) {
    ...
    const code = grayCode(config.cartonCode);
    if (findMaterialByCode(materials, code)) continue;
  ...
  for (const config of VARIANT_CONFIGS) {
    ...
    operations.push({
      ...
      payload: {
        ...
        name: {
          zh: config.nameZh,
          vi: grayNameVi(sourceInfo.name_vi),
        },
        sku: config.sku,
      },
    });
...
function buildReplacementOperations(payload) {
  ...
  for (const config of VARIANT_CONFIGS) {
    ...
    for (const sourceCode of replacements) {
      const source = findMaterialByCode(materials, sourceCode);
      const target = findMaterialByCode(materials, grayCode(sourceCode));
...
export function buildB201ProposalBatches(payload, maxBatchSize = 40) {
  ...
  for (let index = 0; index < operations.length; index += maxBatchSize) {
    ...
    batches.push({
      summary: `ECN-2026-0824-B201: 灰色 SKU 批次 ${batches.length + 1}（${chunk.length} 项操作）`,
      operations: chunk,
    });
  }
  return batches;
