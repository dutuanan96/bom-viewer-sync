...
const FIELD_RULES = Object.freeze({
  ...
  sourceMaterialCodes: stringArray,
  sourceMaterialIds: stringArray,
  sourceSpec: nonEmptyString,
  normalizationFields: stringArray,
  spec: nonEmptyString,
...
export function semanticSchemaPrompt() {
  return [
    ...
    '12. Material consolidation: after duplicate-material evidence, create one consolidate_materials task with sourceMaterialIds and newMaterialCode for each exact group requested by the user, then ask for confirmation. A translation_mismatch/suspected group normally requires a separate Admin confirmation to normalize its fields before it can be consolidated. Exception: if the user explicitly requests ALL audited groups and supplies an unambiguous code convention, create update_material task(s) to normalize only each documented mismatched bilingual field, followed by consolidate_materials for that group; keep old codes and do not delete or disable source materials. Only emit build_proposal after confirmation.',
    '',
    'EXAMPLE:',
