    ...
    try {
      ...
      if (prefetchedCall && runTool) {
        ...
        if (route?.intent === 'proposal' && prefetchedCall.name === 'find_duplicate_materials') {
          ...
          messages.push({
            role: 'user',
            content: `SYSTEM_CONSOLIDATION_CONFIRMATION_REQUIRED: The duplicate-material audit above is the complete bounded evidence for this request. Do not call search_pdm, get_bom, where_used, or any other tool. Output ONLY a Semantic Workflow JSON object with workflowAction "ask_clarification" so the UI asks the user to confirm before any proposal is built. If the user gave a clear code convention from a specification (for example ZK + the numeric dimensions), apply that convention only to duplicate groups with an unambiguous matching specification and create one consolidate_materials task per group using its exact sourceMaterialIds and newMaterialCode. Do not create a code for a group whose specification cannot be mapped unambiguously; ask for that mapping instead.`,
          });
        }
