...
const TEXT = {
  zh: {
    ...
    'ai.agentDecision.duplicate.prompt': '已找到 {exact} 个完全重复组和 {suspected} 个需确认的疑似重复组。请选择下一步；所有变更仍会先生成供管理员审核的建议。',
    'ai.agentDecision.duplicate.exactOnly': '仅处理完全重复组',
    'ai.agentDecision.duplicate.exactOnlyQuery': '请仅为“{materialName}”本次审计中的完全重复物料组创建合并建议。请重新核对 PDM 数据、保留旧物料编码，并在建议中列出每项 BOM 影响。',
    'ai.agentDecision.duplicate.includeSuspected': '包括疑似重复组',
    'ai.agentDecision.duplicate.includeSuspectedQuery': '请为“{materialName}”本次审计中的全部重复组创建合并建议。对每个疑似重复组，先在建议中单独显示需要标准化的字段，再创建对应的合并变更；保留旧物料编码，并重新核对所有 BOM 影响。',
  ...
  vi: {
    ...
    'ai.agentDecision.duplicate.prompt': 'Đã tìm thấy {exact} nhóm trùng hoàn toàn và {suspected} nhóm nghi ngờ cần xác nhận. Hãy chọn bước tiếp theo; mọi thay đổi vẫn phải tạo proposal để Admin duyệt.',
    'ai.agentDecision.duplicate.exactOnly': 'Chỉ xử lý nhóm chắc chắn',
    'ai.agentDecision.duplicate.exactOnlyQuery': 'Chỉ tạo proposal gộp cho các nhóm vật liệu “{materialName}” trùng hoàn toàn trong lần kiểm tra này. Hãy quét lại dữ liệu PDM, giữ nguyên mã vật liệu cũ và liệt kê ảnh hưởng BOM của từng thay đổi.',
    'ai.agentDecision.duplicate.includeSuspected': 'Bao gồm nhóm nghi ngờ',
    'ai.agentDecision.duplicate.includeSuspectedQuery': 'Tạo proposal gộp cho tất cả nhóm vật liệu “{materialName}” trùng trong lần kiểm tra này. Với mỗi nhóm nghi ngờ, hãy hiển thị riêng các trường cần chuẩn hóa trước khi tạo thay đổi gộp; giữ nguyên mã vật liệu cũ và quét lại toàn bộ ảnh hưởng BOM.',
