(function (global) {
  ...
  function findBomAssetEntry(assetMap, material) {
    ...
    const matchers = [
      ...
      ([key]) => {
        const parts = assetParts(key);
        return materialParts.neutralCode && parts.neutralCode === materialParts.neutralCode;
      },
      ([key]) => {
        const parts = assetParts(key);
        return materialParts.name && parts.name === materialParts.name;
      },
      ([key]) => materialParts.code && key.includes(materialParts.code),
      ([key]) => materialParts.comp && key.includes(materialParts.comp)
    ];
