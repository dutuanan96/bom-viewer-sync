(function (global) {
  ...
  function currentPayloadFromWindow() {
    ...
    return normalizePayload({ bom: global.BOM_DATA || {}, drawings: global.DRAWING_INDEX || {}, manuals: global.MANUAL_INDEX || {} });
  }
  ...
  class BomApplication {
    ...
    handleHeaderClick(event) {
      ...
      if (manual) this.openManual(Number(manual.dataset.manualIndex));
    }
    ...
    tableHeadHtml() {
      ...
      return `<tr>${sortable.join('')}<th>${escapeHTML(headers[9])}</th></tr>`;
    }
    ...
    rowHtml(material, index) {
      const cells = ['mat_code', 'comp_code', 'name', 'spec', 'material', 'color', 'attr', 'qty']
        ...
        .join('');
      return `<tr><td>${index + 1}</td>${cells}<td class="drawing-cell">${this.drawingCellHtml(material, index)}</td></tr>`;
    }
    ...
    drawingKey(value) {
      return normalizeText(value).replace(/\.pdf$/i, '').replace(/[\s\-_()（）[\]【】{}+&.]/g, '').replace(/组件/g, '');
    }
    ...
    async writeGithubData(token) {
      ...
      const payload = normalizePayload({ updatedAt, bom: this.state.bom, drawings: this.state.drawings, manuals: this.state.manuals });
    ...
    showModal(url, title, subtitle) {
      ...
      this.query('#pdfFrame').src = this.pdfFrameUrl(url);
