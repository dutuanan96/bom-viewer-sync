...
test.describe('ECN-2026-0710-LGS Admin Proposal UI Flow', () => {
  ...
  test('B201 proposal flow creates all 11 gray SKU variants as local Draft changes', async ({ page }) => {
    ...
    const result = await page.evaluate(() => {
      ...
      return {
        graySkus: variants
          .filter((variant) => expectedSkus.includes(variant.sku) && variant.color === '灰色')
    ...
    expect(result.graySkus).toEqual([
