...
import {
  ...
  VARIANT_CONFIGS,
} from '../src/features/ecn-proposal/b201-gray-variant-proposal-builder.js';
...
function grayCode(code) {
  return `${code.slice(0, -2)}GY`;
}

test('B201 gray variants clone black BOMs and replace only fabric drawers and SKU cartons', async (t) => {
  ...
  await t.test('creates exactly nine reusable gray fabric materials and eleven SKU-specific cartons', () => {
    ...
    for (const sourceCode of fabricCodes) {
      ...
      const target = materialByCode(finalPayload, grayCode(sourceCode));
      assert.ok(target, `${grayCode(sourceCode)} must exist`);
      ...
      assert.deepEqual(target.color, { zh: '灰色', vi: 'màu xám' });
    }
    for (const config of VARIANT_CONFIGS) {
      const source = materialByCode(initialPayload, config.cartonCode);
      const target = materialByCode(finalPayload, grayCode(config.cartonCode));
      assert.ok(target, `${grayCode(config.cartonCode)} must exist`);
      assert.deepEqual(target.spec, source.spec);
      assert.deepEqual(target.material, source.material);
  ...
  await t.test('creates eleven gray SKUs in Draft revisions without changing production effectivity', () => {
    for (const config of VARIANT_CONFIGS) {
      const product = finalPayload.bom[config.spu];
      const color = colorBySku(product, config.sku);
      assert.equal(color, '灰色');
      assert.equal(product.color_info[color].color_ver_vi, 'màu xám');
      ...
      assert.equal(finalPayload.productImages?.[config.spu]?.['灰色'], undefined);
    }
  });
  ...
  await t.test('preserves every BOM row except the confirmed fabric bags and printed carton', () => {
    for (const config of VARIANT_CONFIGS) {
      ...
      const targetEntries = productEntries(finalPayload, config.spu, '灰色');
