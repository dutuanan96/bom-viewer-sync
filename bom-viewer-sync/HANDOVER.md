...
Verify SHA-256 equality for generated artifacts and context documents, then replace the complete Desktop runtime set: `admin.html`, `app-admin.js`, `styles.css`, and `viewer.html`. Do not copy `data.js` or `data/` during code release.
...
- Phase B.6 manual clean-profile `file://` acceptance passed for Viewer and Admin: 22 products, 646 materials, zero console errors, zero `data.js` requests, and no GitHub save. Viewer PDF/GLB and Admin draft-discard behavior were accepted by the operator.
...
- The fresh Phase B.6 closeout baseline passed 194/194 tests; the canonical audit reports 646 materials, 2725 BOM entries, 22 products, 1 notification, 0 errors, and 0 warnings. Publication and release acceptance are complete.
